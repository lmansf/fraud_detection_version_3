...
random_order_types AS (
    ...
    FROM (
        SELECT 
            ...
            row_number() OVER (PARTITION BY bo.batch_row_num ORDER BY rand()) AS rn
        FROM batch_orders bo
        CROSS JOIN (SELECT 'dine_in' AS order_type UNION ALL SELECT 'takeaway' UNION ALL SELECT 'delivery') ot
    )
    WHERE rn = 1
),
random_payment_methods AS (
    ...
    FROM (
        SELECT 
            ...
            row_number() OVER (PARTITION BY bo.batch_row_num ORDER BY rand()) AS rn
        FROM batch_orders bo
        CROSS JOIN (SELECT 'cash' AS payment_method UNION ALL SELECT 'card' UNION ALL SELECT 'wallet') pm
    )
    WHERE rn = 1
),
...
SELECT 
    ...
    cd.cvv as cvv,
    round(random()+0.49,0) as card_match,
